...
void RTC_SetAlarm(RTC_AlarmTypedef* RTC_AlarmStruct) {
	if ((RTC_RegistersStruct.RTC_Alarm1 != Alarm1_Disable)
			| (RTC_RegistersStruct.RTC_Alarm2 != Alarm2_Disable)) {
		if ((RTC_AlarmStruct->seconds >= 60) | (RTC_AlarmStruct->seconds < 0))
			RTC_AlarmStruct->seconds = 0;
		if ((RTC_AlarmStruct->minutes >= 60) | (RTC_AlarmStruct->minutes < 0))
			RTC_AlarmStruct->minutes = 0;
		if ((RTC_AlarmStruct->hours > 24) | (RTC_AlarmStruct->hours < 0))
			RTC_AlarmStruct->hours = 0;
		if ((RTC_AlarmStruct->day > 31) | (RTC_AlarmStruct->day < 0))
			RTC_AlarmStruct->day = 0;
		if ((RTC_AlarmStruct->day_of_week > 7)
				| (RTC_AlarmStruct->day_of_week < 1))
			RTC_AlarmStruct->day_of_week = 0;
		...
		if (RTC_RegistersStruct.RTC_Alarm1 != Alarm1_Disable) {
			...
			if (RTC_RegistersStruct.RTC_Alarm1 == Alarm1_OncePerSec) {
				...
				alarm[3] = A1M4;
			} else if (RTC_RegistersStruct.RTC_Alarm1
